...
UserAndGroup::UserAndGroup() : m_pWbemServices(NULL)
{
    HRESULT hRes;
...
IWbemServices *UserAndGroup::connectServer()
{
    HRESULT hRes;
    ...
    hRes = pIWbemLocator->ConnectServer(L"root\\CIMV2",                 // Namespace
                                        NULL,                           // Userid
                                        NULL,                           // PW
                                        NULL,                           // Locale
...
HRESULT UserAndGroup::enumInstance(const _bstr_t &className,
...
HRESULT UserAndGroup::cacheComputerName()
{
    ...
    HRESULT hRes = enumInstance(
        ...
        [](IWbemClassObject *obj, void *context) -> bool {
            ...
            auto pComputerName = (MyString *)context;
...
HRESULT UserAndGroup::cacheAccount(const _bstr_t &cls, std::vector<struct Account> &var)
{
    HRESULT hRes = enumInstance(
        cls,
        [](IWbemClassObject *obj, void *context) -> bool {
...
HRESULT UserAndGroup::cacheUserGroup()
{
    HRESULT hRes = enumInstance(
        GROUP_USER_CLASS,
        [](IWbemClassObject *obj, void *context) -> bool {
            ...
            std::wsmatch sm;
            MyString strUser = _bstr_t(user), strGroup = _bstr_t(group); // the searched string cannot be temporary.
            MyString userDomain, userAccount, groupDomain, groupName;
            ...
            auto userGroup =
                (std::map<std::pair<MyString, MyString>, std::vector<std::pair<MyString, MyString>>> *)context;
            auto key = std::make_pair(userDomain, userAccount);
            if (userGroup->find(key) == userGroup->end()) {
                (*userGroup)[key] = std::vector<std::pair<MyString, MyString>>();
            }
